...
function calculateApiGravity60Placeholder(observedApi: number, observedTemp: number) {
  // IMPORTANT:
  // This is intentionally NOT doing a fake correction.
  // Accurate API gravity @60 requires the real API/ASTM table engine.
  // Until that engine is added, the app uses the manually entered API @60 value.
  return observedApi || 0
}
...
export default function App() {
  ...
  const [potGravity60, setPotGravity60] = useState('')
  ...
  async function savePotQuality() {
    ...
    const observedGravity = Number(potGravity || 0)
    const observedTemp = Number(potTemp || 0)
    const apiGravity60 = Number(potGravity60 || observedGravity || 0)
    ...
    await supabase.from('tickets').insert({
      ...
      observed_inputs: {
        ...
        observed_api_gravity:
          latestPot?.observed_api_gravity ?? latestPot?.api_gravity ?? null,
        observed_temperature:
          latestPot?.observed_temperature ?? latestPot?.sample_temperature ?? null,
        api_gravity_60:
          latestPot?.api_gravity_60 ?? latestPot?.api_gravity ?? null,
        api_gravity:
          latestPot?.api_gravity_60 ?? latestPot?.api_gravity ?? null,
      ...
      <html>
        ...
        <body>
          ...
          <div class="box">
            ...
            <div class="row"><strong>Observed API Gravity</strong><span>${ticket.observed_inputs?.observed_api_gravity ?? ''}</span></div>
            <div class="row"><strong>Observed Temp</strong><span>${ticket.observed_inputs?.observed_temperature ?? ''}</span></div>
            <div class="row"><strong>API Gravity @60</strong><span>${ticket.observed_inputs?.api_gravity_60 ?? ticket.observed_inputs?.api_gravity ?? ''}</span></div>
        ...
        {page === 'pot' && (
          <>
            ...
            <div style={box}>
              ...
              <input style={input} placeholder="API Gravity @60" value={potGravity60} onChange={(e) => setPotGravity60(e.target.value)} />
              ...
              <div style={card}>Enter API Gravity @60 from table/worksheet until the real API table engine is wired. CSW: {(1 - Number(potBSW || 0) / 100).toFixed(6)}</div>
            ...
            {selectedTicket && (
              <div style={box}>
                ...
                <div style={card}>
                  ...
                  <div>Observed API Gravity: {selectedTicket.observed_inputs?.observed_api_gravity}</div>
                  <div>Observed Temp: {selectedTicket.observed_inputs?.observed_temperature}</div>
                  <div>API Gravity @60: {selectedTicket.observed_inputs?.api_gravity_60 ?? selectedTicket.observed_inputs?.api_gravity}</div>

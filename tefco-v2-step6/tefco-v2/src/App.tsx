...
function addPdfRow(
  ...
  doc.text(pdfText(value), x + width, y)
}

function addPdfSectionTitle(doc: any, title: string, x: number, y: number) {
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(12)
  doc.text(title, x, y)
...
function buildTicketPdfBlob(ticket: Ticket, producers: Producer[]) {
  const doc = new jsPDF({
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(18)
  doc.text('TEFCO Measurement Ticket', 14, y)
  ...
  if (ticket.is_locked) {
    ...
    doc.text('LOCKED CUSTODY TRANSFER RECORD', 125, y)
  ...
  if (ticket.is_superseded) {
    ...
    doc.text('SUPERSEDED TICKET', 125, y)
    doc.setTextColor(0, 0, 0)
  }

  y += 10
  addPdfSectionTitle(doc, 'Ticket Header', 14, y)
  y += 8

  addPdfRow(doc, 'Ticket #', ticket.ticket_number, 14, y, 42)
  addPdfRow(doc, 'Status', ticket.status, 112, y, 35)
  y += 6
  addPdfRow(doc, 'Producer', producer?.name || ticket.producer_id || '', 14, y, 42)
  addPdfRow(doc, 'Type', ticket.ticket_type, 112, y, 35)
  y += 6
  addPdfRow(doc, 'Revision', `R${ticket.revision_number || 0}`, 14, y, 42)
  addPdfRow(doc, 'Approved At', ticket.approved_at || '', 112, y, 35)
  y += 6

  if (ticket.revision_reason) {
    addPdfRow(doc, 'Revision Reason', ticket.revision_reason, 14, y, 42)
    y += 6
  }

  y += 4
  addPdfSectionTitle(doc, 'Observed / Quality Inputs', 14, y)
  y += 8

  addPdfRow(doc, 'Observed API', ticket.observed_inputs?.observed_api_gravity, 14, y, 52)
  addPdfRow(doc, 'Observed Temp', ticket.observed_inputs?.observed_temperature, 112, y, 42)
  y += 6
  addPdfRow(doc, 'API Gravity @60', ticket.observed_inputs?.api_gravity_60 ?? ticket.api_gravity_60, 14, y, 52)
  addPdfRow(doc, 'Density @60', ticket.observed_inputs?.density_60 ?? ticket.density_60, 112, y, 42)
  y += 6
  addPdfRow(doc, 'Avg Temp', ticket.observed_inputs?.average_temperature, 14, y, 52)
  addPdfRow(doc, 'Avg Pressure', ticket.observed_inputs?.average_pressure, 112, y, 42)
  y += 6
  addPdfRow(doc, 'BS&W %', ticket.observed_inputs?.bsw, 14, y, 52)
  addPdfRow(doc, 'CSW', ticket.observed_inputs?.csw, 112, y, 42)

  y += 12
  addPdfSectionTitle(doc, 'Correction Factors', 14, y)
  y += 8

  addPdfRow(doc, 'CTL', ticket.calculation_results?.ctl ?? ticket.ctl, 14, y, 52)
  addPdfRow(doc, 'CPL', ticket.calculation_results?.cpl ?? ticket.cpl, 112, y, 42)
  y += 6
  addPdfRow(doc, 'CTPL / CCF', ticket.calculation_results?.ccf ?? ticket.ctlp, 14, y, 52)
  addPdfRow(doc, 'MF / CMF', ticket.observed_inputs?.meter_factor, 112, y, 42)

  y += 12
  addPdfSectionTitle(doc, 'Volumes', 14, y)
  y += 8

  addPdfRow(doc, 'IV', ticket.observed_inputs?.iv, 14, y, 52)
  addPdfRow(doc, 'GSV', ticket.calculation_results?.gsv, 112, y, 42)
  y += 6
  addPdfRow(doc, 'NSV', ticket.calculation_results?.nsv, 14, y, 52)

  y += 14
  addPdfSectionTitle(doc, 'Calculation Profile Snapshot', 14, y)
  y += 8

  addPdfRow(
    doc,
    'Profile',
    ticket.calculation_profile_snapshot?.contract_profile?.name ||
      ticket.calculation_profile_snapshot?.name ||
      '',
    14,
    y,
    52
  ...
  addPdfRow(
    doc,
    'Method',
    ticket.observed_inputs?.calculation_method ||
      ticket.calculation_profile_snapshot?.selected_calculation_method ||
      '',
    112,
    y,
    42
  ...
  addPdfRow(
    doc,
    'API Engine',
    ticket.observed_inputs?.api_engine ||
      ticket.calculation_results?.api_engine ||
      'API MPMS 11.1',
    14,
    y,
    52
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(
    'This PDF is a generated custody-transfer record snapshot from the approved ticket data stored in the system.',
    14,
    y,
    { maxWidth: 180 }
  )
  ...
  for (let i = 1; i <= pageCount; i += 1) {
    doc.setPage(i)
    doc.setFontSize(8)
    doc.text(`Page ${i} of ${pageCount}`, 180, 270)
    doc.text(`Ticket ${ticket.ticket_number || ticket.id}`, 14, 270)
  }
...
async function generateAndSaveTicketPdf(ticketId: string) {
...
<html>
  ...
  <body>
    ...
    <div>Revision: R${ticket.revision_number || 0}</div>
    <div class="section">
      <h2>Observed Inputs</h2>
      <div class="row"><span class="label">Observed API</span><span>${ticket.observed_inputs?.observed_api_gravity ?? ''}</span></div>
      <div class="row"><span class="label">Observed Temp</span><span>${ticket.observed_inputs?.observed_temperature ?? ''}</span></div>
      <div class="row"><span class="label">API @60</span><span>${ticket.observed_inputs?.api_gravity_60 ?? ticket.api_gravity_60 ?? ''}</span></div>
      <div class="row"><span class="label">Avg Temp</span><span>${ticket.observed_inputs?.average_temperature ?? ''}</span></div>
      <div class="row"><span class="label">Avg Pressure</span><span>${ticket.observed_inputs?.average_pressure ?? ''}</span></div>
    </div>
    <div class="section">
      <h2>Calculation Results</h2>
      <div class="row"><span class="label">CTL</span><span>${ticket.calculation_results?.ctl ?? ticket.ctl ?? ''}</span></div>
      <div class="row"><span class="label">CPL</span><span>${ticket.calculation_results?.cpl ?? ticket.cpl ?? ''}</span></div>
      <div class="row"><span class="label">CTPL/CCF</span><span>${ticket.calculation_results?.ccf ?? ticket.ctlp ?? ''}</span></div>
      <div class="row"><span class="label">GSV</span><span>${ticket.calculation_results?.gsv ?? ''}</span></div>
      <div class="row"><span class="label">NSV</span><span>${ticket.calculation_results?.nsv ?? ''}</span></div>
    </div>
  </body>
</html>
`
      ...
      const safeTicketNumber = String(ticket.ticket_number || ticket.id)
        .replace(/[^a-zA-Z0-9-_]/g, '_')

      const filePath = `${companyId}/${safeTicketNumber}.html`
      ...
      if (uploadError) {
        alert('Could not save ticket PDF: ' + uploadError.message)
        return
      }
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          pdf_url: filePath,
          pdf_file_name: `${safeTicketNumber}.pdf`,
          pdf_generated_at: new Date().toISOString(),
        })
        .eq('id', ticket.id)

      if (updateError) {
        alert('PDF saved, but ticket row did not update: ' + updateError.message)
        return
      }
      ...
      alert('Could not save ticket PDF: ' + (error?.message || String(error)))
    }
  }
          ...
          <>
            ...
            {selectedTicket && (
              <div style={box}>
                ...
                <button style={{ ...button, background: '#16a34a' }} onClick={() => generatePdfPreview(selectedTicket)}>Generate PDF Preview
                  </button>
                  <button
                    style={button}
                    onClick={() =>
                      selectedTicket?.id && generateAndSaveTicketPdf(selectedTicket.id)
                    }
                  >
                    Generate & Save Ticket PDF</button>
                  {selectedTicket?.pdf_url && (
                    <button
                      style={button}
                      onClick={async () => {
                        if (!selectedTicket?.pdf_url) return

                        const { data, error } = await supabase.storage
                          .from('ticket-pdfs')
                          .createSignedUrl(selectedTicket.pdf_url, 60 * 5)

                        if (error || !data?.signedUrl) {
                          alert(error?.message || 'Could not open saved PDF.')
                          return
                        }

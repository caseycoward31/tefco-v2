...
export default function App() {
  ...
  useEffect(() => {
    ...
    const contractProfile = getProducerProfile(
      contractProfiles,
      selectedProducer || latestReading?.producer_id || null
    )
  ...
  async function createTicket() {
    ...
    const corrections = calculateApi11Corrections({
      ...
      apiRounding: Number(contractProfile?.api_rounding || 1),
    })

    const ctl = corrections.ctl
    const cpl = corrections.cpl
    const ctlp = corrections.ctlp
    ...
    const mf = roundTo(
      factorToUse,
      Number(contractProfile?.ctlp_rounding || 4)
    )
    const csw = Number(latestPot?.csw || 1)
    const isApi12 = (contractProfile?.standard || profile?.standard) === 'API 12'

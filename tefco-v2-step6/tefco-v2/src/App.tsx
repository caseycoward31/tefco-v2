...
export default function App() {
  ...
  const [potObservedGravity, setPotObservedGravity] = useState('')
  const [potObservedTemp, setPotObservedTemp] = useState('')
  ...
  async function createTicket() {
    ...
    // FOUNDATION PLACEHOLDER VALUES
    // Real API table interpolation will replace these next phase
    const ctl =
      avgTemp > 60
        ? roundFactor(1 - ((avgTemp - 60) * 0.00035))
        : roundFactor(1 + ((60 - avgTemp) * 0.00035))

    const cpl =
      avgPressure > 0
        ? roundFactor(1 - (avgPressure * 0.00001))
        : 1

    const ctlp = roundFactor(ctl * cpl)
    ...
    await supabase.from('tickets').insert({
      ...
      calculation_method: profile?.name || null,
      ctl,
      cpl,
      ctlp,
      ccf,
      observed_inputs: {
        ...
        observed_api_gravity:
          latestPot?.observed_api_gravity || null,
        observed_temperature:
          latestPot?.observed_temperature || null,
        api_gravity_60:
          latestPot?.api_gravity_60 || null,
        api_gravity:
          latestPot?.api_gravity_60 || null,
            ...
            {selectedTicket && (
              <div style={box}>
                ...
                <div style={card}>
                  ...
                  <div>Stored CTL: {selectedTicket.ctl}</div>
                  <div>Stored CPL: {selectedTicket.cpl}</div>
                  <div>Stored CTLP: {selectedTicket.ctlp}</div>

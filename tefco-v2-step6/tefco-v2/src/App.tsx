...
async function saveUserRole() {
  ...
  async function exportProducerPdfBundle() {
    ...
    const makeTicketPdfBlob = (ticket: any) => {
      ...
      const closeDateTime = `${observed.close_date || ''} ${observed.close_time || ''}`.trim()
      ...
      const lines: [string, any][] = [
        ...
        ['Observed API', numberValue(observed.observed_api_gravity ?? observed.api_observed, 2)],
        ['API @ 60', numberValue(calc.api_gravity_60 ?? observed.api_gravity_60, 2)],
        ['Observed Temp', numberValue(observed.observed_temperature, 2)],
        ['Average Temp', numberValue(calc.average_temperature ?? observed.average_temperature, 2)],
        ['Average Pressure', numberValue(calc.average_pressure ?? observed.average_pressure, 2)],
        ['BS&W', numberValue(calc.bsw_percent ?? observed.bsw_percent ?? observed.bsw, 4)],
        ['BSW', numberValue(calc.csw ?? observed.csw, 5)],
        ['CTL', numberValue(calc.ctl ?? observed.ctl, 6)],
        ['CPL', numberValue(calc.cpl ?? observed.cpl, 6)],
        ['CTPL', numberValue(calc.ctpl ?? observed.ctpl, 6)],
        ['MF / CMF', numberValue(calc.mf ?? observed.mf, 4)],
      ]

      const content: string[] = []
      const writeText = (x: number, yy: number, size: number, text: any, font = 'F1') => {
        content.push(`BT /${font} ${size} Tf ${x} ${yy} Td (${pdfEscape(text)}) Tj ET`)
      }
      const drawLine = (x1: number, y1: number, x2: number, y2: number) => {
        content.push(`${x1} ${y1} m ${x2} ${y2} l S`)
      }

      writeText(42, 748, 22, getCompanyDisplayName(), 'F2')
      writeText(42, 724, 12, 'Measurement Ticket')
      writeText(420, 748, 10, 'TICKET NUMBER')
      writeText(420, 730, 16, ticketNumber, 'F2')
      writeText(420, 712, 10, String(ticket.status || 'approved').toUpperCase())
      drawLine(42, 700, 570, 700)

      let y = 678
      for (const [label, rowValue] of lines) {
        if (y < 70) break
        writeText(48, y, 10, label, 'F2')
        writeText(190, y, 10, rowValue)
        drawLine(42, y - 8, 570, y - 8)
        y -= 20
      }

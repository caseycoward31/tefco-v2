...
const iv = Number(readingClose || 0) - Number(readingOpen || 0)
  ...
  function loadImage(dataUrl: string): Promise<HTMLImageElement> {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => reject(new Error('Could not load proving report image'))
      img.src = dataUrl
    })
  }

  async function createProvingPdfFromPhotos(files: File[]): Promise<Blob> {
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      pdf.addImage(dataUrl, files[index].type.includes('png') ? 'PNG' : 'JPEG', x, y, width, height)
    }
  ...
  async function uploadProvingPdf(provingId: string) {
    ...
    let uploadFile: File | Blob | null = provingPdfFile
    let fileName = provingPdfFile?.name || ''

    if (provingPhotoFiles.length > 0) {
      uploadFile = await createProvingPdfFromPhotos(provingPhotoFiles)
      const meter = meters.find((m) => m.id === provingMeter)
      const meterNumber = (meter?.meter_number || 'proving').replace(/[^a-zA-Z0-9_-]/g, '_')
      fileName = `${meterNumber}_${provingDate || new Date().toISOString().slice(0, 10)}_proving_report.pdf`
    }

    if (!uploadFile) return { pdfUrl: null, fileName: null }
        ...
        {page === 'provings' && (
          <>
            ...
            <div style={box}>
              ...
              <div style={card}>
                <strong>Proving Report Photos → PDF</strong>
                <div style={{ marginTop: 8, color: '#aeb7c2' }}>Take or choose one or more pictures. The app converts them into one PDF.</div>
                <input
                  style={input}
                  type="file"
                  accept="image/*"
                  ...
                  onChange={(e) => {
                    ...
                    setProvingPhotoFiles(files)
                    if (files.length > 0) setProvingPdfFile(null)
                  }}
                />
                {provingPhotoFiles.length > 0 && (
                  <div style={{ marginTop: 8 }}>{provingPhotoFiles.length} photo(s) selected. They will be saved as one PDF.</div>
                )}
              </div>
              <div style={card}>
                <strong>Or Upload Existing Proving PDF</strong>
                <input
                  style={input}
                  type="file"

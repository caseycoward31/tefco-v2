...
export default function App() {
  ...
  useEffect(() => {
    ...
    const profile = profiles.find(
      (p) => p.id === producer?.calculation_profile_id
    )
  ...
  async function createTicket() {
    ...
    const { data: generatedNumber, error } =
      await supabase.rpc(
    ...
    const provingFactor = Number(
      latestApprovedProving?.accepted_meter_factor ||
        0
    )

    const readingFactor = Number(
      latestReading?.meter_factor || 1
    )

    const factorToUse =
      provingFactor || readingFactor || 1
      ...
      observed_inputs: {
        ...
        mf_source:
          latestApprovedProving
            ...
            : 'reading_fallback',
        source:
          'autofill_from_latest_reading_and_proving',
      },
      calculation_results: {
        ccf,
        gsv,
  ...
  function generatePdfPreview(ticket: Ticket) {
    const producer = producers.find(
      (p) => p.id === ticket.producer_id
    )

    const meter = meters.find(
      (m) => m.id === ticket.meter_id
    )

    const segment = segments.find(
      (s) => s.id === ticket.segment_id
    )
      ...
      <html>
        ...
        <body>
          ...
          <div class="box">
            ...
            <div class="row"><strong>Factor Source</strong><span>${ticket.observed_inputs?.mf_source ?? ''}</span></div>
        ...
        {page === 'areas' && (
          <>
            ...
            <div style={box}>
              ...
              />
              <button style={button} onClick={addArea}>
                Add Area
              </button>
            </div>
          ...
          <>
            ...
            <div style={box}>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewProducerProfile(e.target.value)
                }
              >
              ...
              <button style={button} onClick={addProducer}>
                Add Producer
              </button>
            </div>
          ...
          <>
            ...
            <div style={box}>
              <input
                ...
                value={newLeaseName}
                onChange={(e) =>
                  setNewLeaseName(e.target.value)
                }
              />
              <input
                style={input}
                ...
                onChange={(e) =>
                  setNewLeaseNumber(e.target.value)
                }
              />
              ...
              <button style={button} onClick={addLease}>
                Add Lease
              </button>
            </div>
            ...
            {leases.map((l) => (
              <div key={l.id} style={box}>
                <strong>{l.lease_name}</strong>
                <div>{l.lease_number}</div>
              </div>
            ))}
          </>
        )}
          ...
          <>
            ...
            {meters.map((m) => (
              <div key={m.id} style={box}>
                <strong>{m.meter_number}</strong>
                <div>{m.meter_name}</div>
              </div>
            ))}
          </>
        )}
          ...
          <>
            ...
            <div style={box}>
              <select
                ...
                onChange={(e) =>
                  setSelectedReadingMeter(e.target.value)
                }
              >
              ...
              <select
                style={input}
                value={selectedReadingSegment}
                onChange={(e) =>
                  setSelectedReadingSegment(e.target.value)
                }
              >
                <option value="">Select Segment</option>
                {segments.map((s) => (
              ...
              <input
                ...
                value={readingOpen}
                onChange={(e) => setReadingOpen(e.target.value)}
              />
              <input
                style={input}
                placeholder="Closing Reading"
                value={readingClose}
                onChange={(e) => setReadingClose(e.target.value)}
              />
              <input
                style={input}
              ...
              <input
                ...
                placeholder="BS&W %"
                value={readingBSW}
                onChange={(e) => setReadingBSW(e.target.value)}
              />
              <input
                style={input}
                placeholder="Fallback Meter Factor"
                value={readingMF}
                onChange={(e) => setReadingMF(e.target.value)}
              />
              ...
              <button style={button} onClick={saveReading}>
                Save Reading
              </button>
            </div>
          </>
          ...
          <>
            ...
            <div style={box}>
              ...
              <select
                style={input}
                value={provingMeter}
                onChange={(e) => setProvingMeter(e.target.value)}
              >
                <option value="">Select Meter</option>
                {meters.map((m) => (
                  <option key={m.id} value={m.id}>
              ...
              <select
                ...
                onChange={(e) =>
                  setProvingFactorType(e.target.value)
                }
              >
              ...
              <input
                ...
                type="date"
                value={provingDate}
                onChange={(e) => setProvingDate(e.target.value)}
              />
              ...
              <input
                ...
                value={proverVolume}
                onChange={(e) => setProverVolume(e.target.value)}
              />

              <input
                style={input}
                placeholder="Indicated Volume"
                value={provingIndicatedVolume}
                onChange={(e) =>
                  setProvingIndicatedVolume(e.target.value)
                }
              />

              <input
                style={input}
                placeholder="Accepted MF/CMF, blank uses calculated"
                value={acceptedMF}
                onChange={(e) => setAcceptedMF(e.target.value)}
              />
              ...
              <input
                ...
                onChange={(e) => setProvingWitness(e.target.value)}
              />
              ...
              <button style={button} onClick={saveProving}>
                Save Draft Proving
              </button>
            </div>
              ...
              {provings.map((p) => {
                ...
                return (
                  <div key={p.id} style={card}>
                    ...
                    <div>
                      Observed {p.factor_type || 'MF'}:{' '}
                      {Number(p.observed_meter_factor || 0).toFixed(4)}
                    </div>
                    <div>
                      Accepted {p.factor_type || 'MF'}:{' '}
                      {Number(p.accepted_meter_factor || 0).toFixed(4)}
                    </div>
                    ...
                    {p.pdf_url && (
                      <button
                        style={button}
                        onClick={() => viewProvingPdf(p.pdf_url)}
                      >
                        View Proving PDF
                      </button>
                    )}
          ...
          <>
            ...
            <div style={box}>
              ...
              <div style={card}>
                ...
                <div>
                  Profile: {autofillPreview?.profile?.name || 'None'}
                </div>
                <div>
                  IV:{' '}
                  {autofillPreview?.reading?.indicated_volume ??
                    'None'}
                </div>
                <div>
                  Gravity:{' '}
                  {autofillPreview?.reading?.api_gravity ??
                    'None'}
                </div>
                <div>
                  Temp:{' '}
                  {autofillPreview?.reading?.temperature ??
                    'None'}
                </div>
                <div>
                  Latest Approved{' '}
                  {autofillPreview?.proving?.factor_type || 'MF'}:{' '}
                  {autofillPreview?.proving?.accepted_meter_factor ??
                    'None'}
                </div>
                <div>
                  Fallback Reading MF:{' '}
                  {autofillPreview?.reading?.meter_factor ?? 'None'}
                </div>
              </div>
            ...
            <div style={box}>
              ...
              {tickets.map((t) => (
                <div key={t.id} style={card}>
                  ...
                  <div>
                    Factor Type:{' '}
                    {t.observed_inputs?.factor_type || 'MF'}
                  </div>
                  <div>
                    Factor Source:{' '}
                    {t.observed_inputs?.mf_source || 'None'}
                  </div>
            ...
            {selectedTicket && (
              <div style={box}>
                ...
                <div>
                  ...
                  {selectedTicket.calculation_profile_snapshot?.name ||
                    'None'}
                </div>
                ...
                <div style={card}>
                  ...
                  <div>
                    {selectedTicket.observed_inputs?.factor_type || 'MF'}:{' '}
                    {selectedTicket.observed_inputs?.mf}
                  </div>
                  <div>
                    Factor Source:{' '}
                    {selectedTicket.observed_inputs?.mf_source}
                  </div>
                ...
                <button
                  style={button}
                  onClick={() =>
                    updateTicketStatus(selectedTicket, 'submitted')
                  }
                >
                  Submit Ticket
                </button>
                <button
                  style={button}
                  onClick={() =>
                    updateTicketStatus(selectedTicket, 'approved')
                  }
                >
                  Approve Ticket
                </button>
                <button
                  style={button}
                  onClick={() =>
                    updateTicketStatus(selectedTicket, 'draft')
                  }
                >
                  Reject to Draft
                </button>
                <button
                  ...
                  onClick={() =>
                    updateTicketStatus(selectedTicket, 'voided')
                  }
                >
                  Void Ticket
                </button>
                <button
                  ...
                  onClick={() => generatePdfPreview(selectedTicket)}
                >

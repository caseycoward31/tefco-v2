...
const provingCompliancePercent =
  ...
  async function saveCompanySettings() {
    ...
    if (!canEditAdmin) {
      alert('You do not have permission to edit company settings.')
      return
    }
    ...
    if (error) {
      alert('Could not save company settings: ' + error.message)
      return
    }
  ...
  return (
    <div style={{ background: '#020617', color: 'white', minHeight: '100vh', display: 'flex' }}>
      <aside style={{ width: 220, background: '#0f172a', padding: 20 }}>
        ...
        {['dashboard', 'admin', 'areas', 'segments', 'leases', 'producers', 'meters', 'readings', 'pot', 'provings', 'tickets'].map((p) => (
        ...
        {page === 'admin' && canViewAdmin && (
          <>
            ...
            <div style={box}>
              ...
              <p style={{ opacity: 0.8 }}>
                This logo and company information are tied to the current company only.
                It will appear in the app sidebar and on ticket PDFs.
              </p>
              ...
              <input
                style={input}
                placeholder="Small app footer text"
                value={companyFooterInput}
                onChange={(e) => setCompanyFooterInput(e.target.value)}
              />
            ...
            <div style={box}>
              <h2>Company Setup</h2>
              <p style={{ opacity: 0.8 }}>
                Quick setup shortcuts. Detailed edit forms remain below in each setup section.
              </p>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 10 }}>
                <button style={button} onClick={() => setPage('areas')}>Areas</button>
                <button style={button} onClick={() => setPage('segments')}>Segments</button>
                <button style={button} onClick={() => setPage('leases')}>Leases</button>
                <button style={button} onClick={() => setPage('producers')}>Producers</button>
                <button style={button} onClick={() => setPage('meters')}>Meters</button>
              </div>
            </div>

...
export default function App() {
  ...
  const [ticketRevisions, setTicketRevisions] = useState<TicketRevision[]>([])
  const [revisionReason, setRevisionReason] = useState('')
  const [distributionRecipients, setDistributionRecipients] = useState<DistributionRecipient[]>([])
  const [distributionLogs, setDistributionLogs] = useState<DistributionLog[]>([])
  const [newRecipientName, setNewRecipientName] = useState('')
  const [newRecipientEmail, setNewRecipientEmail] = useState('')
  const [newRecipientProducer, setNewRecipientProducer] = useState('')
  const [newRecipientType, setNewRecipientType] = useState('producer')
  const [pdfExportProducer, setPdfExportProducer] = useState('')
  const [pdfExportStartDate, setPdfExportStartDate] = useState('')
  const [pdfExportEndDate, setPdfExportEndDate] = useState('')
  ...
  async function loadAll() {
    ...
    const { data: distributionLogData } = await supabase
    ...
    if (revisionData) setTicketRevisions(revisionData)
    if (recipientData) setDistributionRecipients(recipientData)
    if (distributionLogData) setDistributionLogs(distributionLogData)
...
const provingCompliancePercent =
  ...
  async function saveReading() {
    ...
    if (companySettingsData) {
      ...
      setCompanyNameInput(companySettingsData.company_name || '')
      setCompanyAddress1Input(companySettingsData.address_line1 || '')
      setCompanyAddress2Input(companySettingsData.address_line2 || '')
      setCompanyPhoneInput(companySettingsData.phone || '')
      setCompanyFooterInput(companySettingsData.footer_text || '')
    }
    ...
    const html = `
      <html>
        ...
        <body>
          ...
          <div class="box">
            <div class="row"><strong>Status</strong><span>${ticket.status}{ticket.is_locked ? ' • LOCKED' : ''}{ticket.is_superseded ? ' • SUPERSEDED' : ''}</span></div>
  ...
  async function saveCompanySettings() {
    ...
    if (companyLogoFile) {
      ...
      logoPath = `${companyId}/company-logo.${ext}`
    ...
    const payload = {
      ...
      footer_text: companyFooterInput || null,
      logo_path: logoPath,
      logo_url: logoUrl,
    }
  ...
  function getCompanyDisplayName() {
    return companySettings?.company_name || 'Measurement App'
  }
...
async function saveUserRole() {
  ...
  async function createTicketRevision(ticketId: string) {
    if (!canApproveTickets) {
      alert('You do not have permission to revise tickets.')
      return
    }
    ...
    if (!originalTicket) {
      alert('Original ticket not found.')
      return
    }

    if (!revisionReason) {
      alert('Enter a revision reason.')
      return
    }

    const nextRevision =
      Number(originalTicket.revision_number || 0) + 1
    ...
    const revisionInsert = {
      ...
      original_ticket_id:
        originalTicket.original_ticket_id || originalTicket.id,
    ...
    if (error || !newTicket) {
      alert('Could not create ticket revision.')
      return
    }
    ...
    await supabase
      ...
      .update({
        is_superseded: true,
        superseded_by_ticket_id: newTicket.id,
      })
    ...
    await supabase.from('ticket_revisions').insert({
      original_ticket_id:
        originalTicket.original_ticket_id || originalTicket.id,
      revised_ticket_id: newTicket.id,
      company_id: companyId,
      revision_number: nextRevision,
    ...
    await supabase.from('ticket_audit_log').insert({
      ...
      company_id: companyId,
      action: 'TICKET_REVISED',
      old_status: originalTicket.status,
  ...
  async function exportProducerPdfZip() {
    if (!pdfExportProducer) {
      alert('Select a producer first.')
      return
    }

    if (!pdfExportStartDate || !pdfExportEndDate) {
      alert('Select start and end dates.')
      return
    }

    const producer = producers.find((p) => p.id === pdfExportProducer)

    const matchingTickets = tickets.filter(
      (ticket) =>
        ticket.status === 'approved' &&
        ticket.producer_id === pdfExportProducer &&
        !ticket.is_superseded &&
        ticket.pdf_url &&
        isDateInRange(
          ticket.approved_at || ticket.created_at,
          pdfExportStartDate,
          pdfExportEndDate
        )
    )
    ...
    if (!matchingTickets.length) {
      alert('No approved saved PDFs found for that producer/date range.')
      return
    }
    ...
    for (const ticket of matchingTickets) {
      const { data, error } = await supabase.storage
        .from('ticket-pdfs')
        .createSignedUrl(ticket.pdf_url || '', 60 * 5)

      if (error || !data?.signedUrl) {
        zip.file(
          `${ticket.ticket_number || ticket.id}-ERROR.txt`,
          `Could not create signed URL for ${ticket.ticket_number}: ${
      ...
      if (!response.ok) {
        zip.file(
          `${ticket.ticket_number || ticket.id}-ERROR.txt`,
          `Could not download PDF for ${ticket.ticket_number}`
        )
        continue
      }
      ...
      zip.file(`${safeTicketNumber}.pdf`, blob)
    }
    ...
    const safeProducer = String(producer?.name || 'producer')
      .replace(/[^a-zA-Z0-9-_]/g, '_')

    link.href = url
    link.download = `${safeProducer}_${pdfExportStartDate}_to_${pdfExportEndDate}_ticket_pdfs.zip`
    link.click()
    ...
    await supabase.from('distribution_logs').insert({
      company_id: companyId,
      producer_id: pdfExportProducer,
      status: 'exported',
      notes: `Exported ${matchingTickets.length} ticket PDFs as ZIP for ${producer?.name || pdfExportProducer}`,
      created_by: session?.user?.id || null,
    })
  ...
  async function saveDistributionRecipient() {
    if (!canEditAdmin) {
      alert('You do not have permission.')
      return
    }

    if (!companyId || !newRecipientEmail) {
    ...
    const { error } = await supabase
      ...
      .insert({
        company_id: companyId,
        producer_id: newRecipientProducer || null,
    ...
    setNewRecipientProducer('')
    setNewRecipientType('producer')

    loadAll()
  }

  async function deactivateDistributionRecipient(id: string) {
    if (!canEditAdmin) {
      alert('You do not have permission.')
      return
    }
  ...
  async function queueTicketDistribution(ticketId: string) {
    ...
    const recipients = distributionRecipients.filter(
      (r) =>
        !r.producer_id ||
        r.producer_id === ticket.producer_id
    )

    if (!recipients.length) {
      alert('No recipients configured.')
      return
    }
    ...
    for (const recipient of recipients) {
      await supabase.from('distribution_logs').insert({
        company_id: companyId,
        ticket_id: ticket.id,
        producer_id: ticket.producer_id || null,
        recipient_email: recipient.email,
        status: 'queued',
        notes: 'Queued for email distribution',
        created_by: session?.user?.id || null,
      })
    }

    alert('Ticket distribution queued.')
    loadAll()
  }
  ...
  function exportTicketsCsv() {
    const rows = tickets.map((t) => ({
      ...
      status: t.status,
      revision: t.revision_number || 0,
      producer_id: t.producer_id || '',
      api_60: t.api_gravity_60 || '',
      density_60: t.density_60 || '',
    ...
    <div style={{ background: '#020617', color: 'white', minHeight: '100vh', display: 'flex' }}>
      <aside style={{ width: 220, background: '#0f172a', padding: 20 }}>
        <div style={{ padding: 12, textAlign: 'center' }}>
          {getCompanyLogoUrl() ? (
            <img
              id="company-sidebar-logo"
              src={getCompanyLogoUrl()}
              alt="Company Logo"
              style={{ maxWidth: '100%', maxHeight: 70, objectFit: 'contain' }}
            />
          ) : (
            <h2>{getCompanyDisplayName()}</h2>
          )}
        </div>

        {['dashboard', 'admin', 'reports', 'areas', 'segments', 'leases', 'producers', 'meters', 'readings', 'pot', 'provings', 'tickets'].map((p) => (
      ...
      <main style={{ flex: 1, padding: 30 }}>
          ...
          <>
            ...
            <div style={box}>
              ...
              {selectedTicket?.is_locked &&
                !selectedTicket?.is_superseded &&
                canApproveTickets && (
                  <div style={card}>
                    <h3>Create Revision / Amendment</h3>
                    ...
                    <button
                      style={button}
                      onClick={() =>
                        selectedTicket?.id && createTicketRevision(selectedTicket.id)
                      }
                    >
              ...
              <div style={card}>
                ...
                {ticketRevisions
                  .filter(
                    (r) =>
                      r.original_ticket_id ===
                        (selectedTicket?.original_ticket_id || selectedTicket?.id) ||
                      r.revised_ticket_id === selectedTicket?.id
                  )
                  .map((r) => (
        ...
        {page === 'admin' && canViewAdmin && (
          <>
            ...
            <div style={box}>
              ...
              <input style={input} placeholder="Company Name" value={companyNameInput} onChange={(e) => setCompanyNameInput(e.target.value)} />
              <input style={input} placeholder="Address Line 1" value={companyAddress1Input} onChange={(e) => setCompanyAddress1Input(e.target.value)} />
              <input style={input} placeholder="Address Line 2" value={companyAddress2Input} onChange={(e) => setCompanyAddress2Input(e.target.value)} />
              <input style={input} placeholder="Phone" value={companyPhoneInput} onChange={(e) => setCompanyPhoneInput(e.target.value)} />
              <input style={input} placeholder="Small footer text" value={companyFooterInput} onChange={(e) => setCompanyFooterInput(e.target.value)} />
              <input style={input} type="file" accept="image/*" onChange={(e) => setCompanyLogoFile(e.target.files?.[0] || null)} />
              {getCompanyLogoUrl() && (
                <div style={card}>
                  <strong>Current Logo</strong>
                  <div><img src={getCompanyLogoUrl()} alt="Company Logo" style={{ maxWidth: 260, maxHeight: 120, objectFit: 'contain' }} /></div>
                </div>
              )}
              <button style={button} onClick={saveCompanySettings}>Save Company Branding</button>
            </div>
        ...
        {page === 'reports' && (
          <>
            <h1>Reporting & Distribution</h1>

            <div style={box}>
              <h2>Ticket Exports</h2>

              <button style={button} onClick={exportTicketsCsv}>
                Export Tickets CSV
              </button>

              <div style={{ marginTop: 20 }}>
                <h3>Export Producer PDFs Bundle</h3>

                <select
                  style={input}
                  value={pdfExportProducer}
                  onChange={(e) => setPdfExportProducer(e.target.value)}
                >
                ...
                <input
                  style={input}
                  type="date"
                  value={pdfExportStartDate}
                  onChange={(e) => setPdfExportStartDate(e.target.value)}
                />

                <input
                  style={input}
                  type="date"
                  value={pdfExportEndDate}
                  onChange={(e) => setPdfExportEndDate(e.target.value)}
                />

                <button style={button} onClick={exportProducerPdfZip}>
                  Export Producer PDFs ZIP
                </button>

                <div style={{ marginTop: 10, opacity: 0.8 }}>
                  Only approved, non-superseded tickets with saved PDFs are included.
                </div>
              </div>
            ...
            <div style={box}>
              ...
              <select
                style={input}
                value={newRecipientProducer}
                onChange={(e) => setNewRecipientProducer(e.target.value)}
              >
              ...
              <button
                style={button}
                onClick={saveDistributionRecipient}
              >
                Save Recipient
              </button>
              ...
              <div style={{ marginTop: 20 }}>
                {distributionRecipients.map((recipient) => (
                  <div key={recipient.id} style={card}>
                    <strong>{recipient.name || recipient.email}</strong>

                    <div>Email: {recipient.email}</div>
                    <div>Type: {recipient.recipient_type}</div>

                    <button
                      style={{ ...button, background: '#dc2626' }}
                      onClick={() =>
                        deactivateDistributionRecipient(recipient.id)
                      }
                    >
                      Deactivate
                    </button>
                ...
                .map((ticket) => (
                  <div key={ticket.id} style={card}>
                    ...
                    <button
                      style={button}
                      onClick={() =>
                        queueTicketDistribution(ticket.id)
                      }
                    >
                      Queue Email Distribution
                    </button>
                  </div>
                ))}
            </div>
            ...
            <div style={box}>
              ...
              {distributionLogs.slice(0, 50).map((log) => (
          ...
          <>
            ...
            {selectedTicket && canViewAudit && (
              <div style={box}>
                ...
                <div><strong>Revision:</strong> R{selectedTicket?.revision_number || 0}</div>
                <div><strong>Superseded:</strong> {selectedTicket?.is_superseded ? 'Yes' : 'No'}</div>
                <div><strong>Revision Reason:</strong> {selectedTicket?.revision_reason || 'N/A'}</div>
      ...
      <div style={{ fontSize: 10, opacity: 0.55, textAlign: 'center', padding: 8 }}>
          Powered by TEFCO Measurement Solutions
        </div>
      </main>
    </div>
  )

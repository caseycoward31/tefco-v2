...
export default function App() {
  ...
  async function createTicket() {
    ...
    const ccf = Number(ctl) * Number(ctlp) * Number(mf)
  ...
  return (
    <div style={{ background: '#020617', color: 'white', minHeight: '100vh', display: 'flex' }}>
      <aside style={{ width: 220, background: '#0f172a', padding: 20 }}>
        ...
        {['dashboard', 'producers', 'segments', 'tanks', 'meters', 'readings', 'tickets'].map((p) => (
            ...
            {producers.map((producer) => {
              ...
              return (
                <div key={producer.id} style={box}>
                  <h3>{producer.name}</h3>
                  <div>Profile: {profile?.name || 'None selected'}</div>
                </div>
              )
            })}
          </>
          ...
          <>
            ...
            <div style={box}>
              ...
              <div style={{ marginTop: 15 }}>
                IV: {(Number(readingClose || 0) - Number(readingOpen || 0)).toFixed(2)}
              </div>
            ...
            <div style={box}>
              ...
              {readings.map((r) => (
                <div key={r.id} style={{ background: '#1e293b', padding: 15, borderRadius: 10, marginBottom: 10 }}>
                  <div><strong>IV:</strong> {r.indicated_volume}</div>
                  <div>Gravity: {r.api_gravity}</div>
                  <div>Temp: {r.temperature}</div>
                  <div>BS&W: {r.bsw}</div>
                  <div>MF: {r.meter_factor}</div>
                </div>
              ))}
            </div>
          </>
          ...
          <>
            ...
            <div style={box}>
              ...
              <p style={{ color: '#94a3b8' }}>
                Ticket number auto-generates and cannot duplicate.
              </p>
              ...
              <div style={{ background: '#1e293b', padding: 15, borderRadius: 10, marginTop: 15 }}>
                <h3>Autofill Preview</h3>
                <div>Producer Profile: {autofillPreview?.profile?.name || 'None'}</div>
                <div>Latest Reading IV: {autofillPreview?.reading?.indicated_volume ?? 'None'}</div>
                <div>Gravity: {autofillPreview?.reading?.api_gravity ?? 'None'}</div>
                <div>Temp: {autofillPreview?.reading?.temperature ?? 'None'}</div>
                <div>BS&W: {autofillPreview?.reading?.bsw ?? 'None'}</div>
            ...
            <div style={box}>
              ...
              {tickets.map((t) => (
                <div key={t.id} style={{ background: '#1e293b', padding: 20, borderRadius: 10, marginBottom: 15 }}>
                  ...
                  <div>GSV: {t.calculation_results?.gsv?.toFixed ? t.calculation_results.gsv.toFixed(2) : t.calculation_results?.gsv ?? 'None'}</div>
                  <div>NSV: {t.calculation_results?.nsv?.toFixed ? t.calculation_results.nsv.toFixed(2) : t.calculation_results?.nsv ?? 'None'}</div>
                </div>
              ))}
            </div>
          </>
